---
id: messages
title: Message Capture API
sidebar_label: Message Capture
sidebar_position: 2
---
...
> **Status:** ✅ Documented  
> **Trello Card:** [Endpoints for Sending Message](https://trello.com/c/8lUwoSFh/3147)  
> **Postman:** [Message API Collection](https://lunar-escape-826616.postman.co/workspace/741e2a20-0047-4eba-8e76-85e9bd2f42a2/collection/33570542-96d2421b-9680-4fe1-84b4-3df4ecb33baf)

## Overview

Two endpoints for programmatically capturing messages in workspace chat sessions with optional LLM processing and WebSocket broadcasting.

---

## Endpoints

### 1. Capture Assistant Messages

**Endpoint:** `POST /api/workspace/:workspaceId/session/:sessionId/message/assistant`

**Purpose:** Capture assistant messages (system notifications, automated responses, etc.)

**URL Parameters:**
- `workspaceId` - Workspace UUID or configId
- `sessionId` - Session ID (format: `wses_xxx`) or UUID
...
```http
POST /api/workspace/:workspaceId/session/:sessionId/message/assistant
...
**Note:** The session ID uses the `wses_` prefixed format (e.g., `wses_0cfdac077f0e4498a0e6fdd590df5f0a`), not the long UUID format.
...
**Endpoint:** `POST /api/workspace/:workspaceId/session/:sessionId/message/user`
...
**Use Cases:**
- API-driven chatbot interactions
- Message archiving without processing
- External service integrations
- Multi-channel message capture

#### Request

```http
POST /api/workspace/:workspaceId/session/:sessionId/message/user
Content-Type: application/json
...
**Note:** The `process` flag determines whether the message triggers LLM processing to generate an AI response.
...
### ✅ Flexible Session Resolution
- By session ID (`wses_abc`)
- By session UUID
- Automatic workspace verification

### ✅ Database Persistence
All messages stored in `workspace_session_messages` table with:
...
### 1. API-Driven Chatbot

External service sends user messages and gets AI responses:

```javascript
// User asks a question via external API
const response = await fetch('/api/workspace/79849043-88b5-40ba-a93a-c1e088c4d9b0/session/wses_0cfdac077f0e4498a0e6fdd590df5f0a/message/user', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
...
app.post('/webhook/email/inbound', async (req, res) => {
  ...
  const response = await fetch(
    `/api/workspace/${workspaceId}/session/${sessionId}/message/user`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
- [Chat Streaming API](./streaming.md) - Real-time chat interface
